...
/**
 * The onboarding steps as pure UI, deliberately free of any routing.
 *
 * These used to live only in the /onboarding/ route, reached with a full
 * document navigation, and that turned out to be what broke the published
 * mini app. Inside Toss the static export is served by a host that doesn't
 * resolve a directory URL like "/onboarding/" to its index.html, so the
 * navigation came back as the root document — which promptly fired the same
 * redirect again. The result was an endless reload loop (flickering chrome,
 * a permanent loading screen, nothing clickable) that only appeared once
 * deployed, because the local dev/test path does resolve directory URLs.
 * Rendering these steps in place, from whichever page needs them, keeps the
 * first-run path inside the document it started in.
 */
